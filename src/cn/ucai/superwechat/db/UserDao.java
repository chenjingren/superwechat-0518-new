...
public class UserDao extends SQLiteOpenHelper {
    ...
    public User findUserByUserName(String userName){
        ...
        if (c.moveToNext()){
            int id = c.getInt(c.getColumnIndex(I.User.USER_ID));
            String nick = c.getString(c.getColumnIndex(I.User.NICK));
            String password = c.getString(c.getColumnIndex(I.User.PASSWORD));
            int msg_count = c.getInt(c.getColumnIndex(I.User.UN_READ_MSG_COUNT));
            return new User(id, userName, password, nick, msg_count);
        }
        c.close();
        return null;
    }
    ...
    public boolean updateUser(User user){
        ...
        values.put(I.User.UN_READ_MSG_COUNT,user.getMUserUnreadMsgCount());
        SQLiteDatabase db = getWritableDatabase();
        int update = db.update(TABLE_NAME, values, I.User.USER_NAME + "=?", new String[]{user.getMUserName()});
        return update>0;
    }
}
